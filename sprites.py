...
class Bullet(pg.sprite.Sprite):
    def __init__(self, x: int, y: int, vx: float, vy: float, level: int, img: pg.Surface):
        super().__init__()
        self.image = img
...
class Chicken(pg.sprite.Sprite):
    def __init__(self, x: int, y: int, breed_idx: int, img: pg.Surface, wave_idx: int, breed_data: dict, eggs_group: pg.sprite.Group, egg_img: pg.Surface):
        super().__init__()
        self.image = img
        self.rect = self.image.get_rect(center=(x, y))
        self.dir = 1
    ...
    def update(self, w: int, h: int):
        ...
        shoot_prob = 0.002 + 0.0004 * self.wave
        if random.random() < shoot_prob:
            egg_speed = 4 + 0.2 * self.wave
            new_egg = Egg(self.rect.centerx, self.rect.bottom, 0, egg_speed, self.egg_img)
...
class Boss(pg.sprite.Sprite):
    def __init__(self, x: int, y: int, img: pg.Surface, wave_idx: int, eggs_group: pg.sprite.Group, egg_img: pg.Surface):
        super().__init__()
        self.image = img
        self.rect = self.image.get_rect(center=(x, y))
    ...
    def update(self, w: int, h: int):
        ...
        if self.rect.left < 50 or self.rect.right > w - 50:
            self.dir *= -1
            self.rect.y = min(self.rect.y + 15, h // 3) # Đi dần xuống nhưng không quá 1/3 màn hình
        ...
        if self.shoot_cooldown > 0:
            ...
            if attack_type == "spread":
                ...
                for angle in [-30, -15, 0, 15, 30]:
                    ...
                    self.eggs_group.add(Egg(self.rect.centerx, self.rect.bottom - 20, vx, vy, self.egg_img))
                self.shoot_cooldown = random.randint(90, 150) # Cooldown lâu hơn cho đợt đạn lớn
                
            elif attack_type == "spiral":
                # Kiểu 2: Đợt đạn xoáy ốc (bắn nhanh liên tục 4 quả lệch hướng nhẹ)
                for step in range(4):
                    vx = (step - 1.5) * 1.5
                    self.eggs_group.add(Egg(self.rect.centerx, self.rect.bottom - 20, vx, egg_speed, self.egg_img))
                ...
                self.eggs_group.add(Egg(self.rect.centerx, self.rect.bottom - 20, 0, egg_speed * 1.2, self.egg_img))
                self.eggs_group.add(Egg(self.rect.centerx - 40, self.rect.bottom - 20, -1.0, egg_speed * 1.2, self.egg_img))
                self.eggs_group.add(Egg(self.rect.centerx + 40, self.rect.bottom - 20, 1.0, egg_speed * 1.2, self.egg_img))
...
class Particle(pg.sprite.Sprite):
    ...
    def update(self, w: int, h: int):
        ...
        if self.alpha <= 0:
            ...
            self.image = pg.Surface((self.size, self.size), pg.SRCALPHA)
            pg.draw.circle(self.image, (*self.color, self.alpha), (self.size // 2, self.size // 2), self.size // 2)
...
class Shield(pg.sprite.Sprite):
    ...
    def update(self, w: int, h: int):
        ...
        pg.draw.circle(self.image, (80, 200, 255, alpha), (self.radius, self.radius), self.radius, 3)
        pg.draw.circle(self.image, (80, 200, 255, alpha // 3), (self.radius, self.radius), self.radius - 5)
